...
ПРИОРИТЕТНЫЕ ИСТОЧНИКИ ПО ТЕМАТИКАМ:
- Авто и автосервис: autostat.ru, drive2.ru, нцбд.рф, ФТС России
- IT и технологии: tadviser.ru, cnews.ru, russoft.org
- Промышленность: rosstat.gov.ru, minpromtorg.gov.ru, soyuzmashin.ru
- Финансы: cbr.ru, raexpert.ru, banki.ru
- Ритейл и FMCG: nielsen.com, infoline.spb.ru, retail.ru
- Недвижимость: realty.rbc.ru, cian.ru, дом.рф
- Общие: rosstat.gov.ru, economy.gov.ru, cbr.ru"""
...
function renderResult(i, rawText, structured) {
  ...
  let innerHtml = `<div class="plain-text">${parseMarkdownTables(plainText)}</div>`;
...
// Parse markdown pipe tables into readable HTML tables
function parseMarkdownTables(text) {
  ...
  while (i < lines.length) {
    ...
    if (/^\s*\|/.test(line) && line.trim().endsWith('|')) {
      ...
      let tableLines = [];
      while (i < lines.length && /^\s*\|/.test(lines[i]) && lines[i].trim().endsWith('|')) {
      ...
      html += '<table class="data-table" style="min-width:500px">';
      ...
      rows.forEach((row, ri) => {
        ...
        html += '<tr>' + cells.map((c, ci) => {
          // Bold first column
          const val = escHtml(c.replace(/\*\*/g,'').replace(/\*/g,''));
          const style = ci===0 ? 'font-weight:700;color:#1D9E75' : 'color:#333';
          return `<td style="${style};${bg}">${val}</td>`;
        }).join('') + '</tr>';
      });
      html += '</tbody></table>';
      html += '<button class="excel-link" onclick="exportFile(\'excel\')">';
      html += '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18M9 21V9"/></svg>';
      html += ' Скачать таблицу (Полная версия в Excel)</button></div>';
      result += html;
    } else {
      result += escHtml(lines[i]) + '\n';
      i++;
    }
  }
  return result;
